...
app.post("/api/results", authRequired, async (req, res) => {
  try {
    ...
    /*
     * A learner may check a question, retry it, and submit another answer.
     * The request can therefore contain more than one entry for the same
     * questionIndex. Overwriting the Map entry as we move through the array
     * guarantees that only the learner's LAST attempt is marked and saved.
     */
    ...
    answers.forEach((answer) => {
      ...
      lastAnswerByQuestion.set(questionIndex, answer);
    });
    ...
    const savedAnswers = qs.map((q, i) => {
      ...
      const ans = lastAnswerByQuestion.get(i) || {};
