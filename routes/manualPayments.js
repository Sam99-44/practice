  ...
  async (req, res) => {
    try {
      ...
      if (existingPending) {
        return res.status(400).json({
          message:
            "You already have a pending payment submission awaiting review.",
        });
      }
      ...
      return res.status(201).json({
        message:
          "Proof of payment submitted successfully. Awaiting verification.",
        payment,
      });
    } catch (err) {
  ...
  async (req, res) => {
    try {
      const payments = await ManualPayment.find({})
        .populate("userId", "username email fullName grade")
...
/* =========================================================
   FINANCE: GENERAL UPDATE FOR TABLE DROPDOWNS
  ...
  async (req, res) => {
    try {
      ...
      if ("contacted" in req.body) {
        payment.contacted = !!req.body.contacted;
      }
      ...
      if ("financeStatus" in req.body) {
        payment.financeStatus = normalizeFinanceStatus(req.body.financeStatus);
      }

      if ("financeNotes" in req.body) {
        payment.financeNotes = String(req.body.financeNotes || "").trim();
      }

      if ("rejectionReason" in req.body) {
        payment.rejectionReason = String(req.body.rejectionReason || "").trim();
      }
      ...
      if (payment.financeStatus === "Approved") {
        payment.status = "approved";
        payment.paid = true;
        payment.approvedAt = payment.approvedAt || new Date();
      }

      if (payment.financeStatus === "Rejected") {
      ...
      await writeEmployeeLog(req, "manual_payment_updated", {
        manualPaymentId: payment._id,
        status: payment.status,
        financeStatus: payment.financeStatus,
        contacted: payment.contacted,
        paid: payment.paid,
      });
      ...
      console.error("PATCH /api/manual-payments/:id error:", err);

      return res.status(500).json({
        message: err.message || "Could not update payment.",
      });
    }
  }
);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      await writeEmployeeLog(req, "manual_payment_approved", {
        manualPaymentId: payment._id,
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      await writeEmployeeLog(req, "manual_payment_rejected", {
        manualPaymentId: payment._id,
